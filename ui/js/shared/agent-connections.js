 ...
 *
 * Classic-script module (not ESM) so settings.html can load it via a
 * <script> tag. Exposes `window.kageAgentConnections` as the public API.
 * The welcome window (which uses ES modules) reads the same global.
 *
 * Required globals:
 *   - `window.__TAURI__.core.invoke`
 *   - shared/tool-utils.js loaded first (for escapeHtml on the welcome
 *     side; this file ships its own internal copy so it stays usable
...
(function () {
     ...
     */
    async function renderDetected(container, opts) {
     ...
     */
    function connectionFromDetected(agent) {
        return {
            id: uuidLite(),
     ...
     */
    function renderEditForm(connection, opts) {
        opts = opts || {};
     ...
     */
    function readEditForm(prefix, existing) {
        const sel = document.getElementById(`${prefix}Mode`);
        if (!sel) return null;
        ...
        const mode = isLocal
            ? {
                  type: 'local',
                  spawn_command:
                      document.getElementById(`${prefix}SpawnCommand`)?.value?.trim() || '',
              }
            : {
                  type: 'remote',
