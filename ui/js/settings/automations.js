...
export class AutomationsSettingsModule extends SettingsModule {
        ...
        try {
            ...
            if (invoke) {
                ...
                if (window._extensionManager) {
                    const extTriggers =
                        (await window._extensionManager.getTriggerDefinitions?.()) || [];
                    for (const ext of extTriggers) {
                        for (const t of ext.triggers) {
                            this._signals.push({
                                name: t.name,
                                description: t.description,
                                source: ext.extensionName,
                                icon: t.icon || ext.extensionIcon,
                            });
                        }
                    }
                }
            ...
            console.warn('[Automations] Failed to load signals:', e);
        }
    }
    load(config) {
        ...
        this._powerConfig = config.automation_power || {
            mode: 'auto',
    ...
    save(config) {
        ...
        config.automation_power = this._powerConfig || {
            mode: 'auto',
            battery_multiplier: 2.0,
            low_battery_multiplier: 4.0,
            disable_signals_on_low_battery: false,
    ...
    _markDirty() {
        document
            .getElementById('automationsList')
    ...
    _renderList() {
        ...
        if (this._automations.length === 0) {
            list.innerHTML =
                '<div class="setting-description" style="color:var(--kage-text-muted);font-style:italic">No automations defined yet.</div>';
            return;
        }
        this._automations.forEach((auto, i) => {
            ...
            card.className =
                'auto-card' +
                (auto.enabled === false ? ' disabled' : '') +
                (i === this._expandedIndex ? ' expanded' : '');
    ...
    _expandedCardHtml(auto, i) {
        const outOpts = ['clipboard', 'replace', 'inform']
            .map(
                (v) =>
                    '<option value="' +
                    v +
                    '"' +
                    (auto.output === v ? ' selected' : '') +
                    '>' +
                    { clipboard: '📋 Copy', replace: '✏️ Replace', inform: '💬 Show' }[v] +
                    '</option>'
            )
            .join('');
        const trigTypeOpts = [
            ...
            .map(
                ([v, l]) =>
                    ...
                    v +
                    '"' +
                    ((auto.trigger?.type || 'manual') === v ? ' selected' : '') +
                    '>' +
                    l +
                    '</option>'
            )
        ...
        if (t === 'schedule') {
            ...
            trigConfig =
                '<div style="font-size:11px;color:var(--kage-text-secondary);margin-top:4px;">Runs via inline assist hotkey or quick actions.</div>';
        }
    ...
    _wireExpandedEvents(card, i) {
        card.querySelector('.auto-trigger-type')?.addEventListener('change', () => {
            this._syncExpandedFromDom();
            this._renderList();
        });
        card.querySelector('.auto-schedule-mode')?.addEventListener('change', () => {
            this._syncExpandedFromDom();
            this._renderList();
        });
        ...
        document.getElementById(`autoAddStep_${i}`)?.addEventListener('click', () => {
            this._syncExpandedFromDom();
            this._automations[i].steps.push({
                step_type: 'ai_prompt',
                prompt: '',
                find: '',
                replace: '',
        ...
        document.getElementById(`autoSave_${i}`)?.addEventListener('click', () => {
            ...
            if (err) {
                ...
                if (banner) {
                    banner.className = 'auto-validation-banner';
        ...
        card.querySelectorAll('.auto-step-up').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                const si = parseInt(e.target.closest('.auto-step').dataset.step, 10);
                if (si > 0) {
                    this._syncExpandedFromDom();
                    [this._automations[i].steps[si - 1], this._automations[i].steps[si]] = [
                        this._automations[i].steps[si],
                        this._automations[i].steps[si - 1],
                    ];
                    this._renderList();
                }
            });
        ...
        card.querySelectorAll('.auto-step-down').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                const si = parseInt(e.target.closest('.auto-step').dataset.step, 10);
                if (si < this._automations[i].steps.length - 1) {
                    this._syncExpandedFromDom();
                    [this._automations[i].steps[si], this._automations[i].steps[si + 1]] = [
                        this._automations[i].steps[si + 1],
                        this._automations[i].steps[si],
                    ];
                    this._renderList();
        ...
        card.querySelectorAll('.auto-step-remove').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                if (!this._automations[i].steps.length)
                    this._automations[i].steps.push({
                        step_type: 'ai_prompt',
                        prompt: '',
                        find: '',
    ...
    _syncExpandedFromDom() {
        ...
        this._automations[i].output =
            card.querySelector('.auto-output-select')?.value || 'clipboard';
        ...
        if (trigType === 'manual') {
            ...
            if (mode === 'hourly') {
                parsed.hours = parseInt(card.querySelector('.sched-hours')?.value, 10) || 1;
                parsed.minute = parseInt(card.querySelector('.sched-minute')?.value, 10) || 0;
            } else if (mode === 'daily') {
                parsed.time = card.querySelector('.sched-time')?.value || '09:00';
                parsed.days = Array.from(card.querySelectorAll('.sched-day-btn.active')).map(
                    (b) => b.dataset.day
                );
            } else if (mode === 'monthly') {
                parsed.time = card.querySelector('.sched-time')?.value || '09:00';
                const mm = card.querySelector('.sched-month-mode:checked')?.value || 'day';
                if (mm === 'ordinal') {
                    parsed.dayOfMonth = 0;
                    ...
                    parsed.dayOfMonth =
                        parseInt(card.querySelector('.sched-month-day')?.value, 10) || 1;
                }
            } else if (mode === 'yearly') {
                parsed.time = card.querySelector('.sched-time')?.value || '09:00';
                parsed.month = parseInt(card.querySelector('.sched-year-month')?.value, 10) || 1;
                parsed.monthDay = parseInt(card.querySelector('.sched-year-day')?.value, 10) || 1;
            }
            ...
            this._automations[i].trigger = {
                type: 'signal',
                signal: card.querySelector('.auto-signal-name')?.value || '',
                filter: card.querySelector('.auto-signal-filter')?.value || '',
            };
        }
