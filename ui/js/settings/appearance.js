    ...
    async loadInstalledThemes() {
        ...
        try {
            ...
            for (const t of themes) {
                ...
                // Every theme is uninstallable now — the in-binary
                // "bundled" path is gone, so all themes go through the
                // normal store install/uninstall flow.
                const removeBtn = `<button class="theme-action-btn theme-remove-btn" data-theme-id="${esc(t.manifest.id)}" title="${uninstallBtnTitle}">
