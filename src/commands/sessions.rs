...
/// background AI summarizer is allowed to overwrite it: it only ever
/// touches `Extracted` (or absent) entries — `Manual` (user rename)
/// and `Ai` (already summarized) are off-limits.
...
pub enum TitleSource {
    ...
    Extracted,
    /// Generated by `session_titler::generate_title`. Persists across
    /// further messages; user can override via rename.
    Ai,
...
/// completes successfully. Skips when the title is already locked
/// (`Manual` from rename, `Ai` from a previous summarisation), runs
/// `session_titler::generate_title`, persists the result with
/// `TitleSource::Ai`, and emits `session_changed { renamed }` so all
/// windows refresh.
///
/// Spawns its own background task — caller doesn't await it. The
/// epilogue is the only caller and runs once per message-complete,
/// so retry storms aren't possible: if the agent refuses on this
/// turn, the next user message tries again.
pub fn maybe_generate_ai_title(
    ...
    // Synchronous gate: don't even spawn the task if we already have
    // a locked title. Saves a thread + IPC roundtrip on the common
    // path of "user is replying to an existing summarised conversation".
    {
        let cache = load_title_cache();
        if let Some(entry) = cache.get(&session_id) {
            match entry.source {
                TitleSource::Manual | TitleSource::Ai => return,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let title = match crate::session_titler::generate_title(&client, &session_id) {
            Ok(Some(t)) => t,
            Ok(None) => {
                info!(
