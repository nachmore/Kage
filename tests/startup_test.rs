...
// ---------------------------------------------------------------------------
// cmdline_matches_kage_webview lives in `os::windows::process` (private)
// and is unit-tested inline there with #[cfg(test)]. It used to live here
// for cross-platform test coverage, but on macOS / Linux the surrounding
// Windows-only code path made it dead, which clippy --deny-warnings
// rejected. Native tests next to the implementation are the right home.
// ---------------------------------------------------------------------------
